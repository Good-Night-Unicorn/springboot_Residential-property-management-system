	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="是否绝育" prop="shifoujueyu">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="是否绝育" prop="shifoujueyu">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="宠物详情" prop="chongwuxiangqing">
					<editor 
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.chongwuxiangqing" label="宠物详情" prop="chongwuxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.chongwuxiangqing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(json.loufanghao!=''&&json.loufanghao && this.$storage.get("role")!="管理员"){
						this.ruleForm.loufanghao = json.loufanghao
						this.ro.loufanghao = true;
					}
					if(json.xiaoqumingcheng!=''&&json.xiaoqumingcheng && this.$storage.get("role")!="管理员"){
	...
	this.ruleForm.chongwuxiangqing = this.ruleForm.chongwuxiangqing.replace(reg,'../../../springboot725kw/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yezhuchongwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yezhuchongwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
